...
Sample 18:
Remove the current request from the provider request session.

This sample demonstrates:
- Executing the wrapper CLI from Python
- Removing the current provider request
...
EXIT_REMOVE_REQUEST_FAILED = 3
EXIT_INVALID_USAGE = 4


def build_command(session_id: str) -> list[str]:
    """Build the dotnet CLI command-line arguments for remove-request."""
    return [
        "dotnet",
        str(CLI_DLL),
        "remove-request",
...
def print_usage() -> None:
    ...
    print("python samples/python/18_remove_request.py <session-id>", file=sys.stderr)
...
def main() -> int:
    """Remove the current request from an existing provider request session."""
    ...
    if response["success"] is False:
        print("Remove request failed.")
        print_failure_details(response)
        return EXIT_REMOVE_REQUEST_FAILED
