...
- Closing an ISBM consumer request session
...
EXIT_CLOSE_REQUEST_SESSION_FAILED = 3
EXIT_INVALID_USAGE = 4


def build_command(config: dict[str, str], session_id: str) -> list[str]:
    """Build the dotnet CLI command-line arguments for close-request-session."""
    return [
        "dotnet",
        str(CLI_DLL),
...
def print_usage() -> None:
    ...
    print("python samples/python/14_close_request_session.py <session-id>", file=sys.stderr)
...
def main() -> int:
    ...
    if response["success"] is False:
        print("Close request session failed.")
        print_failure_details(response)
        return EXIT_CLOSE_REQUEST_SESSION_FAILED
    ...
    print("Request session closed successfully.")
    print()
    print("Session ID:")
    print(session_id)
    return EXIT_SUCCESS
