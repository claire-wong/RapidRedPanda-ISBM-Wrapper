...
Sample 02:
Read a publication from an existing subscription session.

This sample demonstrates:
- Executing the wrapper CLI from Python
- Reading publication messages
- Parsing wrapper JSON responses
- Displaying publication content
...
EXIT_READ_PUBLICATION_FAILED = 3
...
def print_usage() -> None:
    ...
    print("python samples/python/02_read_publication.py <session-id>", file=sys.stderr)
...
def display_publication(data: dict[str, object]) -> int:
    """Display publication fields from a successful CLI response."""
    ...
    if not isinstance(topics, list):
        print("The successful CLI response contains invalid 'data.topics'.", file=sys.stderr)
        return EXIT_INVALID_RESPONSE
    ...
    if content is None:
        print("The successful CLI response is missing publication content.", file=sys.stderr)
        return EXIT_INVALID_RESPONSE
...
def main() -> int:
    """Read and display one publication from an existing subscription session."""
    ...
    if response["success"] is False:
        print("Read publication failed.")
        print_failure_details(response)
        return EXIT_READ_PUBLICATION_FAILED
    ...
    if data is None:
        print("No publication available.")
        return EXIT_SUCCESS
    ...
    return display_publication(data)
