...
Sample 04:
Close an existing subscription session.

This sample demonstrates:
- Executing the wrapper CLI from Python
- Closing subscription sessions
...
def print_usage() -> None:
    ...
    print("python samples/python/04_close_subscription.py <session-id>", file=sys.stderr)
...
def main() -> int:
    ...
    if response["success"] is False:
        print("Close subscription failed.")
        print_failure_details(response)
        return EXIT_CLOSE_SUBSCRIPTION_FAILED
    ...
    print("Subscription session closed successfully.")
    print()
    print("Session ID:")
    print(session_id)
    return EXIT_SUCCESS
