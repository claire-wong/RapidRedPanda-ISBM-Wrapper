...
Sample 19:
Close an existing provider request session.

This sample demonstrates:
- Executing the wrapper CLI from Python
- Closing an ISBM provider request session
...
# Use the same ISBM connection values that were used to open the provider request session.
...
EXIT_CLOSE_PROVIDER_REQUEST_SESSION_FAILED = 3
EXIT_INVALID_USAGE = 4


def build_command(session_id: str) -> list[str]:
    """Build the dotnet CLI command-line arguments for close-provider-request-session."""
    return [
        "dotnet",
        str(CLI_DLL),
...
def print_usage() -> None:
    ...
    print("python samples/python/19_close_provider_request_session.py <session-id>", file=sys.stderr)
...
def main() -> int:
    ...
    if response["success"] is False:
        print("Close provider request session failed.")
        print_failure_details(response)
        return EXIT_CLOSE_PROVIDER_REQUEST_SESSION_FAILED
    ...
    print("Provider request session closed successfully.")
    print()
    print("Session ID:")
    print(session_id)
    return EXIT_SUCCESS
